        ...
        {selectedCreative && (
          <>
            <div className="modal-header">
              <h3>{(selectedCreative.ad_name || "Untitled").slice(0, 60)}</h3>
            ...
            <div className="modal-body">
              {selectedCreative.resolved_image_url && (
                <div style={{ marginBottom: 16, textAlign: "center" }}>
                  <img
                    src={selectedCreative.resolved_image_url}
                    alt=""
              ...
              <div className="detail-grid">
                <div className="detail-section">
                  <h4>Performance</h4>
                  <div className="detail-row"><span>Spend</span><span>{fmt(selectedCreative.spend)}</span></div>
                  <div className="detail-row"><span>ROAS</span><span>{selectedCreative.roas.toFixed(2)}x</span></div>
                  <div className="detail-row"><span>CTR</span><span>{selectedCreative.ctr.toFixed(2)}%</span></div>
                  <div className="detail-row"><span>CPA</span><span>{selectedCreative.cpa > 0 ? fmt(selectedCreative.cpa) : "—"}</span></div>
                  <div className="detail-row"><span>CPC</span><span>{fmt(selectedCreative.cpc)}</span></div>
                  <div className="detail-row"><span>Impressions</span><span>{selectedCreative.impressions.toLocaleString()}</span></div>
                  <div className="detail-row"><span>Clicks</span><span>{selectedCreative.clicks.toLocaleString()}</span></div>
                  <div className="detail-row"><span>Purchases</span><span>{selectedCreative.purchases}</span></div>
                  <div className="detail-row"><span>Leads</span><span>{selectedCreative.leads}</span></div>
                </div>

                <div className="detail-section">
                  <h4>Classification</h4>
                  <div className="detail-row"><span>Campaign</span><span>{selectedCreative.campaign_name || "—"}</span></div>
                  <div className="detail-row"><span>Ad Type</span><span>{selectedCreative.ad_type}</span></div>
                  <div className="detail-row"><span>Funnel Stage</span><span>{selectedCreative.funnel_stage || "—"}</span></div>
                  <div className="detail-row"><span>Asset Type</span><span>{selectedCreative.asset_type || "—"}</span></div>
                  <div className="detail-row"><span>Visual Format</span><span>{selectedCreative.visual_format || "—"}</span></div>
                  <div className="detail-row"><span>Messaging Angle</span><span>{selectedCreative.messaging_angle || "—"}</span></div>
                  <div className="detail-row"><span>Hook Tactic</span><span>{selectedCreative.hook_tactic || "—"}</span></div>
                  <div className="detail-row"><span>Offer Type</span><span>{selectedCreative.offer_type || "—"}</span></div>
                </div>
              </div>
              ...
              {selectedCreative.summary && (
                <div className="detail-section" style={{ marginTop: 16 }}>
                  <h4>AI Summary</h4>
                  <p className="helper-text">{selectedCreative.summary}</p>
                </div>
              )}
            </div>
